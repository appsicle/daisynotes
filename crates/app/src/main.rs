//! Daisy Notes — a quiet place to write, with a companion in the margin.
//!
...
fn main() {
    ...
    Application::new()
        .with_assets(daisynotes_ui::assets::DaisyNotesAssets)
        .run(|cx: &mut App| {
            // Fonts must exist before the first frame (no-shift doctrine).
            if let Err(err) = cx.text_system().add_fonts(daisynotes_ui::fonts::all()) {
            ...
            cx.bind_keys(daisynotes_commands::keybindings());
            cx.bind_keys(daisynotes_ui::text_field_bindings());
            // Settings closes on Escape from anywhere in the workspace
            // (the editor's own Escape binding wins while it has focus).
            cx.bind_keys([gpui::KeyBinding::new(
                "escape",
                daisynotes_commands::Cancel,
                Some(daisynotes_commands::WORKSPACE_CONTEXT),
            )]);
            cx.set_menus(daisynotes_commands::app_menus());

            let api = daisynotes_api::spawn();

            let bounds = Bounds::centered(None, size(px(1100.), px(760.)), cx);
            let options = WindowOptions {
                titlebar: Some(TitlebarOptions {
                    title: Some(SharedString::new_static("Daisy Notes")),
            ...
            // Daisy Notes is a single-window app: closing the window quits.
...
/// Logging to `/tmp/daisynotes-debug.log` (truncated on each launch) so it's
...
fn init_tracing() {
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new(
            "daisynotes=debug,daisynotes_api=debug,daisynotes_agent=debug,daisynotes_local=debug,daisynotes_storage=info",
        )
    });

    // Truncate the log on each fresh launch, then open for appending.
    let _ = std::fs::write("/tmp/daisynotes-debug.log", "");
    if let Ok(file) = OpenOptions::new().append(true).open("/tmp/daisynotes-debug.log") {
        ...
        Err(err) => {
            ...
            match Store::open_in_memory() {
                ...
                Err(err) => {
                    ...
                    eprintln!("daisynotes: local storage is unavailable");
                    std::process::exit(1);
                }
            }
        }
